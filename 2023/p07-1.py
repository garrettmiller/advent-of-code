...
def get_hand_type(hand):
    ranks = 'AKQJT98765432'
    rank_count = {r: hand.count(r) for r in ranks}
...
def compare_hands(hand1, hand2):
    ...
    if hand1_type != hand2_type:
        return 'Hand1' if hand1_type > hand2_type else 'Hand2'
...
#Quicksort (thanks ChatGPT)
def quicksort(array):
    if len(array) > 1:
        pivot = array[0]
        greater = [x for x in array if compare_hands(x['hand'], pivot['hand']) == 'Hand2']
        equal = [x for x in array if compare_hands(x['hand'], pivot['hand']) == 'Tie']
        less = [x for x in array if compare_hands(x['hand'], pivot['hand']) == 'Hand1']
        return quicksort(greater) + equal + quicksort(less)
    else:
        return array

# Open the file for reading
with open('p07-input.txt', 'r') as file:
    # Iterate over each line in the file
    for line in file:
        # Split the line into two parts
        hand, bet = line.strip().split()

        # Create a dictionary with hand and bet as key-value pairs
        record = {'hand': hand, 'bet': int(bet)}

        # Add the dictionary to the list
        hands.append(record)

        sorted_array.append(hand)

# Sort the hands based on their strength
sorted_hands = quicksort(hands)

# Calculate the total winnings
total_winnings = sum(hand['bet'] * (index + 1) for index, hand in enumerate(sorted_hands))
